...
// ESTRUTURAS PRONTAS (ativar depois)
async function buscarAliExpress() {
  if (!config.plataformas.aliexpress.ativo) return [];
  return [];
}

async function buscarAmazon() {
  if (!config.plataformas.amazon.ativo) return [];
  return [];
}

async function buscarMercadoLivre() {
  if (!config.plataformas.mercadolivre.ativo) return [];
  return [];
}
