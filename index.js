...
app.get('/automacao', (req, res) => {
  res.json({
    ...
    message: automacaoState.ativa
      ? 'Automação está LIGADA'
      : 'Automação está DESLIGADA',
  });
});
...
app.post('/automacao/toggle', (req, res) => {
  ...
  console.log(`[AUTOMACAO] ${automacaoState.ativa ? 'LIGADA ✅' : 'DESLIGADA ⛔'}`);

  res.json({
    ok: true,
    ativa: automacaoState.ativa,
    message: automacaoState.ativa
      ? 'Automação ligada com sucesso'
      : 'Automação desligada com sucesso',
  });
});

app.post('/automacao/ligar', (req, res) => {
  automacaoState.ativa = true;
  automacaoState.ligadaEm = new Date().toISOString();
  res.json({ ok: true, ativa: true });
});

app.post('/automacao/desligar', (req, res) => {
  automacaoState.ativa = false;
  automacaoState.desligadaEm = new Date().toISOString();
