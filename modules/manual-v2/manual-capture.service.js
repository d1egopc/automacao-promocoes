...
async function gerarPreviewCaptureManualV2(entrada = {}, deps = {}) {
  ...
  const baseConversao = {
    ...
    condicaoPrecoPor,
    precoPix: entrada.precoPix ?? entrada.precoAVista ?? entrada.valorPix ?? "",
    condicaoPix: texto(entrada.condicaoPix),
    frete: texto(entrada.frete || entrada.freteTexto),
    freteValor: entrada.freteValor ?? entrada.valorFrete ?? "",
    taxa: entrada.taxa ?? entrada.imposto ?? entrada.tributo ?? entrada.valorAdicional ?? "",
    imposto: entrada.imposto ?? "",
    moedas: texto(entrada.moedas || entrada.coins),
    linkApp: texto(entrada.linkApp || entrada.urlApp),
  ...
  if (marketplace === "shopee") {
    const integracao = typeof deps.getIntegracaoCliente === "function"
      ...
      : {};
    const credenciais = integracao.credenciais || integracao || {};
    const urlFinalExpandida = await expandirShortlinkShopeeManualV2(urlAfiliada, deps);
